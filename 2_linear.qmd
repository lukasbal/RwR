...
The linear model is the classical workhorse of regression modeling. It is 
easy to interpret and work with in practice and it has tractable theoretical properties. 
It may not be the optimal model choice in any given situation, but it is
a good reference model whenever more sophisticated models are considered, 
and in combination with nonlinear variable transformations and basis expansions, 
as considered in @sec-regression-modeling, it can be surprisingly flexible. 

## Outline and prerequisites 

This chapter introduces the linear model as well as aspects of the general regression modeling 
framework and terminology used throughout the book. An example on fire insurance claim size modeling 
...
model. @sec-model-assessment treats model 
...
The other columns show the values of the variables in the dataset.
...
The `claims` and `sum` variables are numeric, while 
the `grp` variable is a categorical trade group. The trade group is
represented by integers but encoded and stored as a factor.
...
The linear model relates a numerical *outcome*
variable $Y$ to a $p$-dimensional vector $X$ of *predictors* via the identity
$$ 
    \mathbf{E}(Y \mid X) = X^T \beta.
$$ {#eq-linreg}
Here 
$$
...
$\beta$-parameters and $\mathbf{E}(Y \mid X)$ denotes the conditional 
expectation of $Y$ given $X$. Thus the 
linear[^linear-matters] model is a model of the conditional
expectation of the outcome variable given the predictors.

[^linear-matters]: The linearity that matters for estimation and statistical inference is the linearity in the unknown parameter vector $\beta$. It is not linearity in the predictor variables.

An *intercept* parameter, $\beta_0$, is often added,
$$
    \mathbf{E}(Y \mid X) = \beta_0 + X^T \beta.
$$
...
It is, of course, implicit in **A1** and **A2** that $Y$ has finite expectation and variance, 
respectively. While **A1** explicitly specifies the modeling assumption about 
the conditional mean, the purpose of **A2** is less obvious. A good reason to 
want this assumption to hold, is to ensure that the predictors 
primarily affect the outcome distribution via a location shift, and that 
the spread of the distribution is unaffected by the predictors. 
This makes interpretations of the model easier. It also makes assessment of the 
accuracy of model predictions easier. Assumption **A2** has, moreover,
technical consequences for the statistical analysis of data, which we return to
in subsequent parts of the book.

Assumption **A3** is a strong distributional assumption, which entails
**A1** and **A2** but is unnecessarily restrictive for many purposes.
Often **A1** and **A2** are sufficient, but whether **A3** holds
...
denote the $n$ observations of the outcome with corresponding
predictors $X_1, \ldots, X_n$, we collect the outcomes into a column vector $\mathbf{Y}$, and 
...
[The outcome vector and model matrix:
$$
...
$$
    \mathbf{E}(\mathbf{Y} \mid \mathbf{X}) = \mathbf{X} \beta,
$$ {#eq-condmean}
and that 
$$
\mathbf{V}(\mathbf{Y} \mid \mathbf{X}) = \sigma^2 \mathbf{I}
$$ {#eq-condvar}
...
$$ {#eq-conddis}
In summary, there are essentially two sets of distributional assumptions: 

* the weak set **A1**, **A2** and **A4**, which imply the moment identities ([-@eq-condmean]) and
([-@eq-condvar])
* the strong set **A3** and **A5**, which additionally imply the distributional identity ([-@eq-conddis]). 

Occasionally, **A6** combined with **A1** and **A2**
...
be phrased in terms of an *additive error*, which is defined by 
$$
    \varepsilon \coloneqq Y - X^T \beta.
$$ 
[Definition of the *additive error*.]{.aside}

We can then write the outcome in terms of the predictors and the additive error as
$$
    Y = X^T \beta + \varepsilon.
$$
The error term $\varepsilon$ is sometimes also called the 
*noise* term. In terms of $\varepsilon$ the model assumptions can be expressed as: 

*   **A1** is equivalent to $\mathbf{E}(\varepsilon \mid X) = 0$
*   **A2** is equivalent to $\mathbf{V}(\varepsilon \mid X) = \sigma^2$
*   **A3** is equivalent to $\varepsilon \mid X \sim \mathcal{N}(0, \sigma^2)$. 

Note that under assumption **A3**, the conditional distribution of $\varepsilon$ given 
$X$ does not depend upon $X$, thus **A3** implies that $\varepsilon$ and $X$ are independent. 

It is always a challenge in practice to justify model assumptions, and 
we need to accept that assumptions cannot generally be justified 
before the data analysis. There are no magic arguments or
simple statistical summaries that imply that the assumptions are fulfilled. 
A histogram of the marginal distribution of the outcomes can, 
...
model assumptions are generally done *after* a model has been fitted to data,
and this is called *model diagnostics*. 
...
$$\mathbf{E}(\log(Y_i) \mid X_{i, \texttt{sum}}) = \beta_0 + \beta_{\texttt{sum}} \log(X_{i, \texttt{sum}}).$$
...
[The rows in the model matrix shown were chosen to illustrate certain
differences in subsequent examples.]{.aside}
...
the model
[The dummy variable encoding is $$X_{i,\texttt{grp}k} = 1(\texttt{grp}_i = k).$$]{.aside}
$$
\begin{aligned}
\mathbf{E}(\log(Y_i) \mid X_{i, \texttt{sum}}, \texttt{grp}_i) & = \beta_0 + \beta_{\texttt{sum}} \log(X_{i, \texttt{sum}}) \\
...
$$
    \mathbf{E}(\log(Y_i) \mid X_{i, \texttt{sum}} = x, \texttt{grp}_i = 1) = \beta_0 +
    \beta_{\texttt{sum}} \log(x)
$$
and
$$
    \mathbf{E}(\log(Y_i) \mid  X_{i, \texttt{sum}} = x, \texttt{grp}_i = 2) = \beta_0 +
...
write out the eight-dimensional vector explicitly in the condition when in fact
...
predictors we condition on. Suppose that the conditional expectation
$\mathbf{E}(\log(Y_i) \mid X_{i, \texttt{sum}}, \texttt{grp}_i)$ is given by the additive
model, say, then it follows from the tower property of conditional expectations that
$$
\begin{aligned}
\mathbf{E}(\log(Y_i) \mid X_{i, \texttt{sum}}) & = \beta_0
+ \beta_{\texttt{sum}} \log(X_{i, \texttt{sum}}) \\
& + \beta_{\texttt{grp2}} \mathbf{P}(\texttt{grp}_i = 2 \mid X_{i, \texttt{sum}})  \\
& + \beta_{\texttt{grp3}} \mathbf{P}(\texttt{grp}_i = 3 \mid X_{i, \texttt{sum}}) \\
& + \beta_{\texttt{grp4}} \mathbf{P}(\texttt{grp}_i = 4 \mid X_{i, \texttt{sum}}).
\end{aligned}
$$
...
$$ 
    \mathbf{P}(\texttt{grp}_i = k \mid X_{i, \texttt{sum}}) \approx \delta_k + \gamma_k \log(X_{i, \texttt{sum}})
$$
...
+ \beta_{\texttt{sum}}' \log(X_{i, \texttt{sum}})
$$
will be an approximately valid model as well with 
$$
\begin{align*}
    \beta_0' &= \beta_0 + \beta_{\texttt{grp2}} \delta_2 + 
                 \beta_{\texttt{grp3}} \delta_3 +  \beta_{\texttt{grp4}} \delta_4 \\
    \beta_{\texttt{sum}}' & =  \beta_{\texttt{sum}} + \beta_{\texttt{grp2}} \gamma_2 + 
                 \beta_{\texttt{grp3}} \gamma_3 +  \beta_{\texttt{grp4}} \gamma_4. 
\end{align*}
$$
That is, even if we do not condition on the group, the model is (approximately) valid, 
but the intercept and slope parameters for this
model may differ from those in the model where we additionally condition on the group. This depends
on whether the $\beta_{\texttt{grp}k}$, $\delta_k$ and
...
always depending upon exactly which variables we condition on, whether this is
...
methods we will consider in @sec-generalized-linear.

We introduce the *weighted squared error loss*
[With $\mathbf{W} = \mathbf{I}$ this loss is proportional to 
the negative log-likelihood loss under assumptions **A3** and **A5** as derived in @sec-generalized-linear.]{.aside}
\index{loss!squared error} \index{weighted squared loss} as 
$$ 
 \mathrm{Loss}(\beta) = (\mathbf{Y} - \mathbf{X}\beta)^T\mathbf{W}(\mathbf{Y}
- \mathbf{X}\beta)
$$ {#eq-wsel}
...
$$
     \mathrm{Loss}(\beta) =  \sum_{i} w_i (Y_i - X_i^T \beta)^2.
$$
That is, the $i$-th case receives the weight $w_i$. 
\index{weight matrix} \index{penalty matrix}

We will estimate the parameter $\beta$ via minimization of $\mathrm{Loss}$---or via 
minimization of $\mathrm{Loss}$ plus a penalty term. With $\boldsymbol{\Omega}$ 
a positive semidefinite $p \times p$ matrix we will seek to minimize the function  
$$ 
 \mathrm{Loss}_{\boldsymbol{\Omega}}(\beta) :=  \mathrm{Loss}(\beta) + \beta^T \boldsymbol{\Omega} \beta.
$$ {#eq-penll}
...
beneficial numerical as well as statistical consequences. The purpose of introducing a penalty will be elaborated upon in subsequent chapters, where some concrete examples will be given.   
...
::: {.proof} 
The derivative of $\mathrm{Loss}_{\boldsymbol{\Omega}}$ is 
$$
    D_{\beta}  \mathrm{Loss}_{\boldsymbol{\Omega}}(\beta) = -2(\mathbf{Y} - \mathbf{X}\beta)^T \mathbf{W} \mathbf{X} + 2 \beta^T \boldsymbol{\Omega}.
$$

To compute this derivative, it may be useful to think of $\mathrm{Loss}$ as a
composition, 
$$
    \mathrm{Loss}(\beta) = b(a(\beta)),
$$ 
where $a(\beta) = (\mathbf{Y} - \mathbf{X}\beta)$ and $b(z) = z^T \mathbf{W} z.$
Here $a$ is a map $a : \mathbb{R}^{p} \to \mathbb{R}^n$ with
derivative $D_{\beta}a(\beta) = -\mathbf{X}$. And $b$ is a function $b : \mathbb{R}^n \to \mathbb{R}$ 
with derivative $D_z b(z) = 2 z^T \mathbf{W}.$ By the chain rule
$$ 
    D_{\beta}  \mathrm{Loss}(\beta) = D_zb(a(\beta))D_{\beta}a(\beta) = -2(\mathbf{Y} - \mathbf{X}\beta)^T \mathbf{W} \mathbf{X}.
$$
...
[^column-gradient]: The *gradient*, $$\nabla_{\beta}   \mathrm{Loss}(\beta) = D_{\beta}  \mathrm{Loss}(\beta)^T,$$ is a column vector.

We note that @eq-normal is equivalent to 
$D_{\beta}  \mathrm{Loss}_{\boldsymbol{\Omega}}(\beta) = 0,$ 
hence a solution of @eq-normal is a stationary point of $\mathrm{Loss}_{\boldsymbol{\Omega}}.$

The second derivative of $\mathrm{Loss}_{\boldsymbol{\Omega}}$ is 
$$
    D_{\beta}^2  \mathrm{Loss}_{\boldsymbol{\Omega}}(\beta) = 
...
$D_{\beta}^2  \mathrm{Loss}_{\boldsymbol{\Omega}}(\beta)$ is positive definite and the minimizer of 
$\mathrm{Loss}_{\boldsymbol{\Omega}}$ is the unique solution of @eq-normal.
:::
...
we see that 
$$
     \mathrm{Loss}(\beta) = \|\mathbf{Y} - \mathbf{X}\beta\|^2_{\mathbf{W}}.
$$ 
If $L = \{\mathbf{X}\beta  \mid \beta \in \mathbb{R}^p\}$ denotes the column
space of $\mathbf{X}$, $\mathrm{Loss}$ is minimized whenever $\mathbf{X}\beta$ is the 
...
$$
    \boldsymbol{\Pi} = \mathbf{X} (\mathbf{X}^T \mathbf{W} \mathbf{X})^{-1} \mathbf{X}^T \mathbf{W} 
$$
provided that $\mathbf{X}$ has full column rank $p$. 
:::

::: {.proof}
We verify that $\boldsymbol{\Pi}$ is the orthogonal projection onto $L$ by verifying three
characterizing properties: 
$$
    \begin{align*}
        \boldsymbol{\Pi} \mathbf{X} \beta & = \mathbf{X} \beta \quad (\boldsymbol{\Pi} \text{ is the identity on } L) \\
        \boldsymbol{\Pi}^2 & = \mathbf{X} (\mathbf{X}^T \mathbf{W} \mathbf{X})^{-1} 
        \mathbf{X}^T \mathbf{W}  \mathbf{X} 
        (\mathbf{X}^T \mathbf{W} \mathbf{X})^{-1} \mathbf{X}^T
        \mathbf{W} \\
        & = \mathbf{X}
        (\mathbf{X}^T \mathbf{W} \mathbf{X})^{-1} \mathbf{X}^T \mathbf{W} = \boldsymbol{\Pi} \\
        \boldsymbol{\Pi}^T \mathbf{W}  
        ...
        \mathbf{W} = \mathbf{W} \boldsymbol{\Pi}.
    \end{align*}
$$
...
This follows from the identity $\boldsymbol{\Pi}\mathbf{Y} = \mathbf{X} \hat{\beta}$, 
since the equation $\boldsymbol{\Pi}\mathbf{Y} = \mathbf{X} \beta$ has a unique solution
...
$$
    \boldsymbol{\Pi} = \mathbf{X} (\mathbf{X}^T \mathbf{W} \mathbf{X})^{-} \mathbf{X}^T \mathbf{W}
$$
where $(\mathbf{X}^T \mathbf{W} \mathbf{X})^{-}$ denotes a generalized inverse.
[A generalized inverse of a matrix $\mathbf{A}$ is any matrix $\mathbf{A}^{-}$ with the property that $$\mathbf{A}\mathbf{A}^{-}\mathbf{A} = \mathbf{A}.$$]{.aside}
This is seen by verifying the same three conditions as in the proof
above. The solution to $\boldsymbol{\Pi}\mathbf{Y} = \mathbf{X} \beta$ is, however, no longer unique, and the 
solution 
$$
...
The computations based on the QR decomposition do not involve the computation 
...
$$ 
    ...
    \mathbf{X}\hat{\beta}\|^2
$$ {#eq-varest} 
provided that $\mathbf{X}$ has full rank $p$. Defining the $i$-th *residual* as
\index{residual!linear model}
...
natural estimator of the unobserved error $\varepsilon_i$, the
...
is related to the fact that $\hat{\varepsilon}_i$ is an estimator of $\varepsilon_i$.
...
$$
    \begin{align*}
    \mathbf{E}(\hat{\beta} \mid \mathbf{X}) & = \beta, \\
    \mathbf{V}(\hat{\beta} \mid \mathbf{X}) & = \sigma^2 (\mathbf{X}^T\mathbf{X})^{-1}, \\
    \mathbf{E}(\hat{\sigma}^2 \mid \mathbf{X}) &= \sigma^2.
    \end{align*}
$$
:::
...
$$
    \begin{align*}
    \mathbf{E}(\hat{\beta} \mid \mathbf{X}) & = 
    \mathbf{E}((\mathbf{X}^T\mathbf{X})^{-1} \mathbf{X}^T \mathbf{Y} \mid \mathbf{X}) \\
    & = (\mathbf{X}^T\mathbf{X})^{-1} \mathbf{X}^T \mathbf{E}(\mathbf{Y} \mid \mathbf{X}) \\
...
$$
    \begin{align*}
    \mathbf{V}(\hat{\beta} \mid \mathbf{X}) & = (\mathbf{X}^T\mathbf{X})^{-1} \mathbf{X}^T \mathbf{V}(\mathbf{Y} \mid \mathbf{X})  \mathbf{X} (\mathbf{X}^T\mathbf{X})^{-1} \\
...
interpretation of $\hat{\beta}$ is useful. Since $\mathbf{X} \hat{\beta} = \boldsymbol{\Pi} \mathbf{Y}$
with $\boldsymbol{\Pi}$ the orthogonal projection onto the column space $L$ of $\mathbf{X}$, we find that 
$$
    \mathbf{Y} - \mathbf{X} \hat{\beta} = (\mathbf{I} - \boldsymbol{\Pi}) \mathbf{Y}.
$$
Because $\mathbf{E}(\mathbf{Y} - \mathbf{X} \hat{\beta} \mid \mathbf{X}) = 0$ 
$$
    \mathbf{E}(\|\mathbf{Y} - \mathbf{X} \hat{\beta}\|^2 \mid \mathbf{X}) = \sum_{i=1}^n \mathbf{V}(\mathbf{Y} - \mathbf{X} \hat{\beta} \mid \mathbf{X})_{ii}
$$
and  
$$
    \begin{align*}
    \mathbf{V}(\mathbf{Y} - \mathbf{X} \hat{\beta} \mid \mathbf{X}) & = \mathbf{V}((\mathbf{I} - \boldsymbol{\Pi}) \mathbf{Y} \mid \mathbf{X}) \\
    & = (\mathbf{I} - \boldsymbol{\Pi}) \mathbf{V}(\mathbf{Y} \mid \mathbf{X}) (\mathbf{I} - \boldsymbol{\Pi})^T \\
    & = (\mathbf{I} - \boldsymbol{\Pi}) \sigma^2 \mathbf{I} (\mathbf{I} - \boldsymbol{\Pi}) \\
    & = \sigma^2 (\mathbf{I} - \boldsymbol{\Pi}).
    \end{align*}
$$
The sum of the diagonal elements in $(\mathbf{I} - \boldsymbol{\Pi})$ is the trace of this 
...
$$ 
    a^T\hat{\beta} \pm z_{n-p} \cdot \hat{\sigma} \sqrt{a^T(\mathbf{X}^T\mathbf{X})^{-1}a}
$$ {#eq-confint}
...
$$ {#eq-subspace}
for a $p \times p_0$ matrix $\mathbf{C}$ (of rank $p_0$), and we can verify that
$L' \subseteq L$ by finding a $\mathbf{C}$ matrix such that ([-@eq-subspace]) holds.
The situation where the columns of $\mathbf{X}'$ is a subset of the columns of $\mathbf{X}$, which is
a hypothesis on the complementary set of parameters being $0$,
corresponds to $\mathbf{C}$ having $0$-s or $1$-s appropriately placed
along the main diagonal and off-diagonal elements being $0$.
[It is not necessary to compute the actual $\mathbf{C}$ matrix
...
In the light of George Box's famous aphorism, *all models are wrong but some are 
useful*, it is appropriate to add a supplementary
Box quote: *Since all models are wrong the scientist must be alert to what is
importantly wrong* [@Box:1976]. This is done by investigating the assumptions
**A1**, **A2** and **A3** *after* the model has been fitted to data and typically via the residuals
$$
    \hat{\varepsilon}_i = Y_i - X_i^T \hat{\beta},
$$ {#eq-resid-def} \index{residual!linear model}
which are approximations of the errors $\varepsilon_i$.

We use the residuals to investigate the model assumptions mostly via graphical methods. 

* Plots of $\hat{\varepsilon}_i$ against a predictor or against the *fitted values* $X_i^T \hat{\beta}$ 
  show deviations from assumptions **A1** or **A2**
* A qq-plot of the residuals $\hat{\varepsilon}_i$ against the normal distribution shows deviations 
  from assumption **A3**.

\index{residual sum of squares}
To measure the predictive strength of a linear model, we can compute the residual sum of squares 
$$
    \mathrm{RSS} = \|\mathbf{Y} - \mathbf{X} \hat{\beta}\|^2 = \sum_{i=1}^n (\hat{\varepsilon}_i)^2.
$$
It quantifies how well $X_i^T \hat{\beta}$ predicts $Y_i$ for $i = 1, \ldots, n$. 
The absolute value of $\mathrm{RSS}$ is difficult to interpret, and it 
also suffers from an optimism by being computed on the data used for 
...
With a single numerical predictor, $R^2$ is the *square of the Pearson
  correlation* \index{Pearson correlation!and $R^2$}
  between the outcome and the predictor. 
...
$$
    \mathbf{E}\left((Y^{\mathrm{new}} - (X^{\mathrm{new}})^T \hat{\beta})^2 \mid  \hat{\beta} \right),
$$
...
### Investigating the claim size models {#sec-claim-size-models}
...
inhomogeneity. The histogram of the residuals and the qq-plot of the standardized[^standardize] residuals
...
[^standardize]:Standardized so as to make the residuals have unit variance under the model assumptions.
...
We proceed by looking at the model summary. 
...
R function `summary()` prints a summary of that information. 
...
size. Note that for this regression model with only one numerical predictor,
...
#| code-fold: show
#| tbl-cap: "Parameter estimates for the simple linear model of claim sizes"
#| tbl-cap-location: bottom
tidy(claims_lm) |>
    knitr::kable()
```

We then turn to the additive model that includes the trade group effect.
We first consider the parameter estimates for the additive model. 
Recall that we used the sum contrast for the (final version of the) model. 

```{r}
#| label: tbl-claims-add-tidy
#| tbl-cap: "Parameter estimates for the additive model of claim sizes"
#| tbl-cap-location: bottom
tidy(claims_lm_add) |>
    knitr::kable()
```
...
For the interaction model, see @tbl-claims-int-glance,  the $R^2$ has 
increased a little compared to the additive model, but 
three additional parameters have also been added to the model. If we look at the adjusted $R^2$ 
...
The conclusion is that we cannot reject $H_1$ while $H_0$ can be clearly rejected. 
Thus using this approach, data does not provide evidence that the slope should 
be different in the four trade groups, but there is strong evidence that the 
intercept should. This is in concordance with the observations made above based 
on the estimated parameters and $R^2$. 

We conclude the section by revisiting the diagnostic plots, but this 
time for the interaction model, see @fig-claims-diag-int.
...
The conclusion is the same as for the simpler model that only includes insurance
sum as predictor. The mean value model is
not spot on and the residual distribution is slightly skewed. It is not a bad
...
and the data. We do not usually call this function ourselves, but understanding 
...
#| fig-cap: "The left figure shows a scatter plot of the `y` variable against the `x` variable. The right figure adds color coding according to the group as encoded by the `z` variable. The dashed curves are the true means within the three groups."
#| cap-location: margin
#| echo: false
...
1.  Fit a linear regression model using `lm(y ~ x, formula_data)` where `y` is the outcome variable 
...
We happen to know that the data was generated with a mean value structure 
that is actually a linear model in $\sin(x)$ and $\cos(x)$. 

5. Run the following code and explain what it returns.

```{r}
#| label: correct-model-formula
#| code-fold: show
#| eval: false
...
6. Fit the model specified by the formula `y ~ cos(x) + z * sin(x)` to the simulated 
data, explain the parameter estimates you get and reproduce @fig-formula-linear-model-one.

```{r}
#| label: fig-formula-linear-model-one
#| opts.label: widefigure
#| echo: false
#| message: false
...
#| cap-location: margin
y_lm <- lm(y ~ cos(x) + z * sin(x), formula_data)
...
2.  Fit a piecewise affine function with the "bend" at 50% of the 
    ...
    *Hint: you want to fit a linear model with a model matrix $\mathbf{X}$ with
    three columns representing an intercept and
    two piecewise linear functions. You can, for instance, generate those piecewise 
    linear functions by an
    `ifelse()` construction in the formula. Try different choices of columns. Is there 
    one choice particularly suitable for testing the linearity assumption?*

::: {#exr-column-space}
Show that if ([-@eq-subspace]) holds then 
$$\mathbf{C} = (\mathbf{X}^T \mathbf{X})^{-1} \mathbf{X}^T \mathbf{X}'.$$
...
::: {#exr-gauss-markov}
Let $\mathbf{C}$ be any $n \times p$ matrix and consider the *linear estimator* 
$$
    \tilde{\beta} = \mathbf{C}^T \mathbf{Y}.
$$
:::
1.  Show that $\tilde{\beta}$ is unbiased as an estimator of $\beta$ if **A1** and **A4** hold and if 
   $$\beta = \mathbf{C}^T \mathbf{X} \beta$$
    for all $\beta$.
2.  Show that if the estimator is unbiased and **A1**, **A2** and **A4** hold then 
    $$\mathbf{V}(\tilde{\beta} \mid \mathbf{X}) = \sigma^2 \mathbf{C}^T\mathbf{C}$$
    and that
    $$\mathbf{V}(\hat{\beta}- \tilde{\beta} \mid \mathbf{X}) = \sigma^2 (\mathbf{C}^T\mathbf{C} - (\mathbf{X}^T\mathbf{X})^{-1})$$
